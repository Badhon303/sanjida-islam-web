  ...
  return (
    <article className="group h-full flex flex-col rounded-2xl overflow-hidden relative">
      <div
        ...
        } rounded-2xl blur opacity-0 group-hover:opacity-30 transition-all duration-300`}
      ></div>
      <div className="relative bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 h-full flex flex-col border border-slate-200 overflow-hidden">
        {/* Image/Header */}
        {image ? (
          <div className="relative w-full h-64 bg-linear-to-br from-emerald-50 to-green-50 overflow-hidden flex items-center justify-center">
            <Image
              src={image}
              alt={title}
              fill
          ...
          <div
            className={`relative h-64 bg-linear-to-br ${
              isPast
          ...
          >
            <FaCalendar
              className={`text-6xl ${
                isPast ? "text-slate-400" : "text-emerald-300"
              }`}
            />
            {isPast && hasVideo && (
              <div className="absolute top-4 right-4 px-3 py-1.5 bg-red-600 text-white rounded-full text-xs font-bold shadow-lg flex items-center gap-1.5 z-10">
        ...
        <div className="flex-1 flex flex-col p-6">
          ...
          {/* Title */}
          <h3
            className={`text-xl font-bold mb-3 group-hover:text-emerald-600 transition-colors line-clamp-2 ${
