...
export default function BlogCard({
  ...
  return (
    <article className="group h-full flex flex-col rounded-2xl overflow-hidden relative">
      <div
        className={`absolute inset-0 bg-linear-to-r ${colorGradient} rounded-2xl blur opacity-0 group-hover:opacity-30 transition-all`}
      ></div>
      <div className="relative bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all h-full flex flex-col border border-slate-200">
        {/* Category Badge */}
        <div className={`relative bg-linear-to-br ${colorGradient} px-6 py-8`}>
        ...
        <div className="flex-1 flex flex-col p-6">
          ...
          <Link
            href={href}
            className={`inline-flex items-center gap-2 text-sm font-bold bg-linear-to-r ${colorGradient} bg-clip-text text-transparent hover:opacity-80 transition-all`}
          >
            আরও পড়ুন
